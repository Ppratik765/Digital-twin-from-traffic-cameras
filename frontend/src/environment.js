...
export class DigitalTwinEnvironment {
  constructor(scene) {
    ...
    // Color palette carefully calibrated to match the real video footage
    this.materials = {
      grass: new THREE.MeshStandardMaterial({ color: 0x426738, roughness: 0.9, metalness: 0.05 }),
      asphalt: new THREE.MeshStandardMaterial({ color: 0x33383f, roughness: 0.85, metalness: 0.1 }),
      concreteRoad: new THREE.MeshStandardMaterial({ color: 0x9ca3af, roughness: 0.8, metalness: 0.05 }), // Light grey concrete pavement
      concreteJoint: new THREE.MeshBasicMaterial({ color: 0x6b7280 }), // Expansion slab joints
      ...
      houseWall1: new THREE.MeshStandardMaterial({ color: 0xf8fafc, roughness: 0.7 }), // White siding
      houseWall2: new THREE.MeshStandardMaterial({ color: 0xd6cbb8, roughness: 0.8 }), // Warm beige siding
      houseWall3: new THREE.MeshStandardMaterial({ color: 0x94a3b8, roughness: 0.7 }), // Slate siding
      houseWall4: new THREE.MeshStandardMaterial({ color: 0xb5c0ad, roughness: 0.8 }), // Sage green siding
  ...
  buildRoadNetwork() {
    // 1. Main East-West Highway Corridor
    // Spans Z from -92m to +32m (Width 124m, Center Z = -30m), Length 1600m
    ...
    // 2. Intersecting North-South Arterial
    // Spans X from -42m to +64m (Width 106m, Center X = +11m), Length 1600m
    ...
    // EW Sidewalks (North side Z = -95, South side Z = +35)
    ...
    // NS Sidewalks (West side X = -45, East side X = +67)
  ...
  buildSuburbanHouses() {
    // Positioned safely outside all vehicle lanes
    const houseConfigs = [
  ...
  buildTrees() {
    // Positioned safely outside all traffic lanes (verified 0 collisions)
    const treePositions = [
